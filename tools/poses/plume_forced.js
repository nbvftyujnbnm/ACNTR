...
(async () => {
  ...
  // FORCE the plumes far past any plausible visibility threshold. Position and
  // direction are now confirmed correct (mains fire backward from an anchor
  // 6 m up, 1.4 m behind), intensity is 0.62, instanceCount is 4, and nothing
  // appears. This separates "drawn but too dim" from "not drawn at all": if a
  // plume at intensity 6 with a 1.2 m radius is still invisible, the draw is
  // not happening and no amount of tuning will help.
  // Detach the per-frame driver FIRST. Game._updatePlayerThrusters runs every
  // frame and calls handle.set(true, level), which resets target to 0.62 — and
  // step() does not render, so the captured frame is one the harness draws about
  // a second of real time later, by which point the force is long gone. The
  // first attempt at this test measured nothing for exactly that reason.
  game._plumes = null;
