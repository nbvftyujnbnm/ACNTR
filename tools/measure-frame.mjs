...
/* -------------------------------------------------------------------------
 * PNG reading lives in `tools/png.mjs`, which has NO side effects on import.
 * It is re-exported here so existing callers keep working — but import it from
 * `./png.mjs` in anything new: this file ends in a top-level IIFE that builds,
 * launches a browser and calls `process.exit(0)`, so importing ANYTHING from
 * here runs a capture harness and then kills the importing process.
 * ---------------------------------------------------------------------- */
